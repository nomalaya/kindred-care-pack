...
const SHARE_TEXT = "Je viens d'aider une personne dans le besoin via CashForCause. Rejoignez-moi !";
...
const PostDonSocialBlock = () => {
  ...
  const handleMobileShare = async () => {
    try {
      await navigator.share({
        title: "J'ai aidé via CashForCause",
        text: SHARE_TEXT,
        url: SHARE_URL,
      });
    } catch {
  ...
  const copyAndOpen = (url: string, platform: string) => {
    navigator.clipboard.writeText(`${SHARE_TEXT} ${SHARE_URL}`);
    window.open(url, "_blank", "noopener,noreferrer");
    toast.success(`Texte copié — collez-le dans votre ${platform}`);
  };
        ...
        {isMobile ? (
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="center" className="w-56">
              ...
              <DropdownMenuItem onClick={() => copyAndOpen(SOCIAL_LINKS.instagram, "story Instagram")} className="cursor-pointer gap-2">
              ...
              <DropdownMenuItem onClick={() => copyAndOpen(SOCIAL_LINKS.tiktok, "vidéo TikTok")} className="cursor-pointer gap-2">
