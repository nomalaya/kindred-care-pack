...
// ─── Active circle with integrated pulse ───
const ActiveCircle = ({ icon: Icon, extraClass = "" }: { icon: React.ElementType; extraClass?: string }) => (
  <motion.div
    animate={PULSE_SHADOW.animate}
    transition={PULSE_SHADOW.transition}
    className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 bg-primary text-primary-foreground ${extraClass}`}
  >
    ...
    <div className={`relative ${isMobile ? "flex flex-col gap-0 pl-2" : ""}`}>
      {!isMobile && (
        <div className="absolute top-5 left-5 right-5 h-0.5 bg-border z-0">
          <motion.div className="h-full bg-primary" initial={{ width: 0 }} animate={{ width: "10%" }} transition={{ duration: 1, ease: "easeOut" }} />
        </div>
      )}
      {isMobile && (
        <div className="absolute left-[18px] top-5 bottom-5 w-0.5 bg-border z-0">
          <motion.div className="w-full bg-primary" initial={{ height: 0 }} animate={{ height: "15%" }} transition={{ duration: 1, ease: "easeOut" }} />
        </div>
      )}
      ...
      <div className={isMobile ? "flex flex-col gap-4" : "flex items-start justify-between relative z-10"}>
        {STEPS.map((step, i) => {
          ...
          return (
            ...
            >
              {isActive ? (
                <ActiveCircle icon={Icon} extraClass="border-2 border-primary" />
              ) : (
                <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 bg-background text-primary/50 border-primary/20">
                  <Icon className="h-5 w-5" />
                </div>
              )}
            ...
            return (
              ...
              >
                {isActive ? (
                  ...
                  <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 bg-primary/10 text-primary/50 border-primary/30">
                    <Icon className="h-5 w-5" />
                  </div>
                )}
                <div className={isMobile ? "pt-1" : "text-center mt-2"}>
        ...
        <div className={isMobile ? "flex flex-col gap-4" : "flex items-start justify-between relative z-10"}>
          {STEPS.map((step, i) => {
            ...
            return (
              ...
              >
                {isActive ? (
                  <ActiveCircle icon={Icon} extraClass="shadow-warm" />
                ) : (
                  <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-card bg-background text-primary/50 border border-primary/20">
                    <Icon className="h-5 w-5" />
                  </div>
                )}
                <div className={isMobile ? "pt-1" : "text-center mt-2"}>
        ...
        {STEPS.map((step, i) => {
          ...
          return (
            <motion.div
              ...
              } ${isMobile ? "flex items-start gap-4" : "flex flex-col items-center text-center flex-1"}`}
            >
              {isActive ? (
    ...
    <Layout>
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-10">
        <div>
          ...
          <p className="text-muted-foreground mt-1">5 variations design de la timeline de rassurance — sans flèches, pulse intégré</p>
        </div>
