...
FORBIDDEN: tight face crop, passport photo, sloped narrow shoulders, cropped shoulders, head filling the canvas, cropped forehead, cropped hair, hair touching the top edge, flattened afro, squashed hair volume, cropped headscarf or hat, small distant subject floating in white, white border around the subject, floating head effect, small triangle of clothing under the chin, full torso, waist visible, ribcage visible, hips, full-length arms, hands, deep cleavage, exposed chest skin beyond a normal neckline.
COMPOSITION: subject centered horizontally, looking softly toward the camera.
CORPORATE STAFF-DIRECTORY STUDIO — STRICT: every portrait of the catalog is shot in the SAME studio setup: same camera distance, same lens height (at eye level), same neutral lighting, same plain white backdrop. The head must occupy the SAME share of the frame for everyone. The morphology is NEVER altered to match the frame: a broad build stays broad, a slim build stays slim.
HEADROOM — STRICT: the ENTIRE hair volume (afro, braids, locs, bun, long hair) and any head covering (headscarf, veil, hijab, cap, hat) is FULLY INSIDE the frame with a small band of free background above it. NEVER crop, flatten, shrink or squash the hair or the head covering to fit — step the camera back instead.
